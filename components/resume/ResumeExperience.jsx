...
const ResumeExperience = () => {
  ...
  if (!translations.resume.experienceList || translations.resume.experienceList.length === 0) {
    return null;
  }

  const totalPages = Math.ceil(translations.resume.experienceList.length / itemsPerPage);
  const startIndex = (currentPage - 1) * itemsPerPage;
  const currentItems = translations.resume.experienceList.slice(
    startIndex,
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="text-center mb-8">
        ...
        <p className="text-blue-400/60">{translations.resume.experienceDescription}</p>
      </div>

      {/* Experience Grid */}
      <div className="grid grid-cols-1 gap-6">
        {currentItems.map((item, index) => (
          <ExperienceItem key={startIndex + index} item={item} index={index} />
        ))}
      </div>
