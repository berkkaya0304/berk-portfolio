...
const ResumeVoluntarily = ({ voluntarilyWorks = { items: [] } }) => {
  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 3;

  // Mevcut sayfadaki gönüllü işleri al
  const indexOfLastItem = currentPage * itemsPerPage;
  const indexOfFirstItem = indexOfLastItem - itemsPerPage;
  const currentItems = voluntarilyWorks.items.slice(indexOfFirstItem, indexOfLastItem);

  // Toplam sayfa sayısını hesapla
  const totalPages = Math.ceil(voluntarilyWorks.items.length / itemsPerPage);
  ...
  if (!voluntarilyWorks?.items || voluntarilyWorks.items.length === 0) {
    return null;
  }
    ...
    <div className="space-y-8">
      ...
      <div className="text-center mb-8">
        <h2 className="text-2xl font-bold mb-2 bg-clip-text text-transparent bg-gradient-to-r from-blue-400 to-blue-700">
          {voluntarilyWorks.title}
        </h2>
        <p className="text-blue-400/60">{voluntarilyWorks.description}</p>
      </div>
