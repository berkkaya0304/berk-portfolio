      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        >
          ...
          <h1 className="text-4xl md:text-5xl font-bold mb-4 bg-clip-text text-transparent bg-gradient-to-r from-blue-400 to-blue-700">
            {translations.social.theaters.title}
          </h1>
          <p className="text-lg text-blue-400/80 max-w-2xl mx-auto">
            {translations.social.theaters.description}
          </p>
          <div className="mt-6 flex justify-center gap-4">
            <div className="flex flex-col items-center p-4 bg-slate-900/50 rounded-xl border border-blue-400/20">
              <span className="text-2xl font-bold text-blue-400">{theaters.length}</span>
              <span className="text-sm text-blue-300/80">{translations.social.theaters.totalShows}</span>
            </div>
            <div className="flex flex-col items-center p-4 bg-slate-900/50 rounded-xl border border-blue-400/20">
              <span className="text-2xl font-bold text-blue-400">
                {theaters.reduce((total, theater) => {
                  const rating = parseFloat(theater.Rating);
                  return isNaN(rating) ? total : total + rating;
                }, 0) / theaters.length || 0}
              </span>
              <span className="text-sm text-blue-300/80">{translations.social.theaters.averageRating}</span>
            </div>
          </div>
        </motion.div>
